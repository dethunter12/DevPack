  ...
  public class IconPickerSkill : Form
  {
    ...
    private void IconPicker_Load(object sender, EventArgs e)
    {
      ...
      for (int index = 0; index < ((IEnumerable<string>) files).Count<string>(); ++index)
      {
        ...
        if (((IEnumerable<string>) strArray).Count<string>() == 2 && strArray[1].ToLower().Contains("skillbtn"))
                    comboBox1.Items.Add( Path.GetFileNameWithoutExtension(files[index]));
      }
      if (comboBox1.Items.Count > 0)
                comboBox1.SelectedIndex = 0;
            comboBox1.SelectedItem =  ("SkillBtn" + OldItemBtnSelect);
    }
    ...
    private void pictureBox1_MouseClick(object sender, MouseEventArgs e)
    {
      int x = e.X;
      int y = e.Y;
            TexID = Convert.ToInt32(comboBox1.SelectedItem.ToString().Remove(0, 8));
            TexRow = (int) Math.Floor((double) y / 32.0);
            TexColumn = (int) Math.Floor((double) x / 32.0);
            SelectedIcon.Image = (Image)databaseHandle.IconSkill1(TexID, TexRow, TexColumn);
    }
    ...
    {
            pictureBox1.Image = Image.FromFile("icons/" + comboBox1.SelectedItem + ".png");
    }
    ...
    {
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(IconPickerSkill));
